...
export default async function handler(req: any, res: any) {
  ...
  try {
    // 1. Resolve Brand Logo path for inline embedding
    const logoPath = path.join(process.cwd(), 'public/images/brands/doctor-in-logo-1.png');
    const hasLocalLogo = fs.existsSync(logoPath);
    const logoSrc = hasLocalLogo ? 'cid:doctorinlogo' : 'https://doctor-in.com/images/brands/doctor-in-logo-1.png';

    const attachments = [];
    if (hasLocalLogo) {
      attachments.push({
        filename: 'doctor-in-logo-1.png',
        path: logoPath,
        cid: 'doctorinlogo',
      });
    }
    ...
    const transporter = nodemailer.createTransport({
      ...
      tls: {
        rejectUnauthorized: false, // Prevents SSL/TLS validation failures locally
      },
    });
    ...
    // 2. Build HTML Body Content per Form Type using Brand Colors (#071f5d secondary navy, #f42434 primary coral)
    if (type === 'contact') {
      ...
      htmlBodyContent = `
        ...
        <div style="text-align: center; margin-top: 30px;">
          ${waLink ? `
            <a href="${waLink}" target="_blank" style="background-color: #25D366; color: #ffffff; padding: 14px 28px; border-radius: 30px; text-decoration: none; font-weight: bold; font-size: 14px; display: inline-block; margin: 5px; box-shadow: 0 4px 12px rgba(37, 211, 102, 0.25);">Chat via WhatsApp 💬</a>
          ` : ''}
    ...
    // 3. Assemble complete HTML Template wrapper
            ...
            <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 30px rgba(7, 31, 93, 0.08); border: 1px solid #e2e8f0;">
              ...
              <div style="background-color: #071f5d; padding: 35px 30px; text-align: center; color: #ffffff; font-size: 13px;">
                ...
                <div style="margin-bottom: 25px;">
                  <a href="https://wa.me/51941667151" style="background-color: #25D366; color: #ffffff; padding: 12px 24px; border-radius: 9999px; text-decoration: none; font-weight: bold; font-size: 12px; display: inline-block; margin: 5px; box-shadow: 0 4px 8px rgba(37, 211, 102, 0.2);">WhatsApp Emergency</a>
    ...
    // 4. Send the mail (forwarding form contents to info@doctorcusco.com)
    const mailOptions = {
